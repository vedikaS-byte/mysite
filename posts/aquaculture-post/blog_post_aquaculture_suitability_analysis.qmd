...
vector EEZ polygons are needed to operate in raster space so area can be
...
raster values inside EEZ regions and exclude suitable locations outside
these boundaries. The output was a raster containing the surface area of
cells using `cellSize()`. Finally, I used `zonal()` to sum the area of
...
`kableExtra` to display both the total suitable area and the proportion
of each EEZ that is suitable for oyster aquaculture, enabling comparison
...
`species_preference` function which applies the generalized workflow
...
It is important to note that `species_preference` only considers
...
the `species_preference` function to generate a map of suitable
