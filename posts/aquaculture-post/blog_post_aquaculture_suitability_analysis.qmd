...
#author: "" 
description: "GIS-based suitability analysis for shellfish aquaculture"
categories: [MEDS, data-viz, R]
date: "12/6/2025"
...
The West Coast includes multiple Exclusive Economic Zones (EEZs), ocean
areas extending roughly 230 miles beyond a nation’s territorial sea
where a coastal nation has jurisdiction over both biotic and abiotic
resources (NOAA, 2023). Both oysters and red abalone are known to
inhabit waters along the U.S. West Coast. As such, identifying regions
...
sst_files <- list.files(
  path = here::here("posts", "aquaculture-post", "data_aqua"),
...
#avg_sst_depth[is.na(avg_sst_depth)] <- 0
...
oyster_pref_map <- tm_shape(area_eez) +
  tm_polygons(
   ...
   title = expression("Suitable Area"~"("~ km^2~")"), # Rename legend title
  colorNA = "transparent",
  textNA = "",
...
predation) are not included and are thus simplified. The accuracy of the
